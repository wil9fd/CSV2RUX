...
class Funcs():
    ...
    def import_inputs(self):
        ...
        for key in vars(self):
            ...
            if isinstance(getattr(self, key), PyQt5.QtWidgets.QLineEdit):
                self.inputs.update({key: getattr(self, key).text()})
            # As above but for comboboxes
            elif isinstance(getattr(self, key), PyQt5.QtWidgets.QComboBox):
                self.inputs.update({key: getattr(self, key).currentText()})
            # As above but for double spin boxes
            elif isinstance(getattr(self, key), PyQt5.QtWidgets.QDoubleSpinBox):
    ...
    def get_delimiter(self):
        ...
        try:
            ...
            if Path(self.inputs['inputFile']).exists() is True:
                # Read it with pandas
                self.input_df = pd.read_csv(self.inputs['inputFile'], sep = self.input_delimiter, header=None)
    ...
    def test_id(self):
        ...
        if any([is_true_float(num) for num in re.split(';|\s', str(self.input_df[0]))]) is True: 
            self.has_id = False
        else:
            self.has_id = True
    ...
    def test_delimiter(self):
        ...
        if str(self.dialect.delimiter) != self.inputs['delimiter']:
            ...
            if self.delim_button == PyQt5.QtWidgets.QMessageBox.Close:
    ...
    def convert2ddd(self):
        ...
        def dms2ddd(element):
            ...
            deg, minutes, seconds, direction =  re.split(';|\s', element)
        ...
        def dmm2ddd(element):
            ...
            deg, minutes, direction =  re.split(';|\s', element)
        ...
        self.input_df = pd.concat([self.input_df.iloc[:,0], self.LL_df], axis=1, ignore_index=True)
    ...
    def remove_dups(self, informat):
        ...
        if self.input_df.shape[1] == 5 and self.informat.currentText() == informat:
            ...
            self.input_df = pd.DataFrame(self.df_value_list, columns = ['WPName','Lat','Lon'])
    ...
    def calculate_radius(self):
        ...
        for index in range(1,(len(self.input_df)-1)):
            ...
            ll_1 = list(utm.from_latlon(self.point_list[index-1][0], self.point_list[index-1][1]))
            ll_2 = list(utm.from_latlon(self.point_list[index][0], self.point_list[index][1]))
            ll_3 = list(utm.from_latlon(self.point_list[index+1][0], self.point_list[index+1][1]))
    ...
    def format_rux(self):
        ...
        self.input_df.insert(0, column = 'Id', value = np.arange(self.input_df.shape[0])+1)
        
        # If there was no id
        if self.has_id == False:
            # Set the wpname as id too
            self.input_df['WPName'] = np.arange(self.input_df.shape[0])+1
    ...
    def make_plot(self):
        ...
        for value in self.input_df["Id"].tolist():
            ...
            mysymbol.addText(0, 0, QtGui.QFont("San Serif", 10), str(value))
    ...
    def write_rux(self):
        ...
        # If multiple rux's are wanted then create a new folder and store them in there
            ...
            folder_name = Path(os.path.dirname(self.inputs['outputFile']) + "\\" + self.file_tag)
            
            # IF the folder doesn't already exist make it
            if folder_name.exists() is False:
                os.mkdir(os.path.dirname(self.inputs['outputFile']) + "\\" + self.file_tag)
        ...
        if self.inputs['fledermausfile'] == 'Yes':            
            ...
            with open(fm_filename, 'w') as fm_file:
                ...
                for lat,lon in self.point_list: 
                    ...
                    fm_file.write(str('{:},{:}\n'.format(np.array2string(lat),np.array2string(lon))))
